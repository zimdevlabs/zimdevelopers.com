...
export default async function ProfilePage({
  ...
  searchParams: Promise<{ username?: string; viewMode?: string }>;
}) {
  // This would normally come from authentication
  const { viewMode } = await searchParams;
  const isOwnProfile = viewMode !== "visitor";
  const userDetails = mockUser;

  return (
    <div className="w-full px-4 py-8">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
        <div className="w-full overflow-hidden rounded-lg bg-white shadow lg:col-span-3">
          {/* Header */}
          <div className="flex w-full flex-col items-center border-b p-6 text-center">
            <div className="h-24 w-24 overflow-hidden rounded-full bg-gray-100">
              {userDetails.avatar ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-blue-100 text-xl font-semibold text-blue-500">
            ...
            {userDetails.role && (
              <span
                className={`mt-2 rounded-full px-2 py-1 text-xs text-white ${getRoleBadgeColor(userDetails.role)}`}
              >
                {userDetails.role}
              </span>
            )}
            {userDetails.speciality && (
              <span className="mt-2 rounded-full border border-gray-300 px-2 py-1 text-xs">
                {userDetails.speciality}
              </span>
            )}
          </div>
            ...
            <div className="flex flex-col items-center space-y-4 text-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">
                  Member since {userDetails.createdAt.toLocaleDateString()}
                </span>
              </div>
          ...
          <div className="border-t p-6">
            ...
            {isOwnProfile && !userDetails.profileCompleted && (
              <div className="mt-2 rounded border border-yellow-300 bg-yellow-50 p-2 text-center text-sm text-yellow-700">
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow lg:col-span-9">
            ...
            <>
              ...
              <div className="p-6">
                <div className="space-y-4 text-center">
                  <p>
                    You&apos;re viewing {userDetails.name}&apos;s public
                    profile.
                  </p>
